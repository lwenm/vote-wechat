...
/**
	插屏广告onload
*/
util.ad_cp = function (pagekey) {
  var app = getApp();
  var pages = getCurrentPages();
  var nowPage = pages[pages.length - 1];
  var c_time = new Date().getTime();
  console.log(app.globalData);
  if (app.globalData && app.globalData.ad_cp.enabled[pagekey] && app.globalData.ad_cp.id[pagekey]) {
    console.log(1111111);
    if (!app.globalData.ad_cp_time) {
      app.globalData.ad_cp_time = 0;
    }
    if ((c_time - app.globalData.ad_cp_time) > app.globalData.ad_cp.time) {
      if (nowPage.data.ad_cp) {
        console.log(2222);
        nowPage.data.ad_cp.show().catch((err) => {
          console.error(err)
        })
        app.globalData.ad_cp_time = new Date().getTime();
      } else {
        console.log(333333);
        if (wx.createInterstitialAd) {
          nowPage.data.ad_cp = wx.createInterstitialAd({ adUnitId: app.globalData.ad_cp.id[pagekey] })
          nowPage.data.ad_cp.onLoad(() => {
            console.log('onLoad event emit ')
          })
          nowPage.data.ad_cp.onError((err) => {
            console.log('onError event emit ', err)
          })
          nowPage.data.ad_cp.onClose((res) => {
...
/*二次封装request请求，确保每次请求都保证能带上sessionid*/

util.request = function (option) {
 
  var login_option = option;
  var _ = require('underscore.js');
  var md5 = require('md5.js');
  var app = getApp();
  ...
  if (!sessionid) {
    ...
    wx.login({
      success: function (res) {
        var login_code = { code: res.code };
        var login_url = util.url('auth/session/openid');
        login_url = login_url + '&m=' + 'tech_vote';
        var sign = getSign(login_url, login_code);
        if (sign) {
          login_url = login_url + "&sign=" + sign;
        }
        wx.request({
          url: login_url,
          data: login_code,
...
* 兼容iPhoneX XS MAX
*/
util.resizeFooter=function($this){
  var that=$this;
  wx.getSystemInfo({
    success(res) {
      if (res.model.indexOf("iPhone X") != -1 || res.model.indexOf("iPhone XS") != -1) {
        that.setData({
          isIPhoneX: true
        })
      } else {
...
//设置导航栏的背景色和标题
util.setNBC = function(fontcolor,color){
